...
WT_Expected_input = 0.0084.*WTWT.*WTWT.*WTWT - 0.2176.*WTWT.*WTWT + 24.865.*WTWT + 39.196 ; % Note : wrong but fitted model

KO_Expected_input = 0.0084.*KOKO.*KOKO.*KOKO - 0.2176.*KOKO.*KOKO + 24.865.*KOKO + 39.196 ; % Note : wrong but fitted model


 Codename = 'PoisInputFr150324_newExp';
% distribution of expected input
myl = 850; %myl = max(max(Input_FR_WT(:)), max(Input_FR_KO(:)));
