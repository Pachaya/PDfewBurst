...
X = WTWT;
WT_Expected_input = 147.3 * exp(0.006698.*X) - 147.4 ; 

X= KOKO;
KO_Expected_input = 147.3 * exp(0.006698.*X) - 147.4 ; 


 Codename = 'PoisInputFr150403_newExp';
 dirLoc = [Codename '/']; 
 mkdir(dirLoc)
% distribution of expected input
myl = 35; %myl = max(max(Input_FR_WT(:)), max(Input_FR_KO(:)));
