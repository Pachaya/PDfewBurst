...
Codename = 'PoisInputFr150327';
WT_Expected_input = 11.43.*exp(0.06084.*WTWT);
KO_Expected_input =11.43.*exp(0.06084.*KOKO);

% Ideal Cases 
yy =  11.43.*exp(0.06084.*xx);
figure; plot(xx,yy,'.-k'); hold on; 
scatter(WTWT,WT_Expected_input,'b');
scatter(KOKO,KO_Expected_input,'r');
xlabel('Output Frequency (Hz)');
ylabel('Input Frequency (Hz)');
title('Output - Input relationship in ideal case');
legend('f(x) = 11.43*exp(0.06084*x)','WT','KO')
