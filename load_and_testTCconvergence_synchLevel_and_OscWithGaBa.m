...
for p3_ii = 1 : length(PARAM3)
    ...
    for p4_ii = 1 : length(PARAM4)
        for p5_ii = 1 : length(PARAM5)
            ...
            x[ fg_M1FRnumVL ] = plotFigure_paramMat( M1_baselineWT,M1_baselineKO, tt1,tt2, xl,yl, xt,yt, suptitleTxt,figLoc );
...
for p4_ii = 1 : length(PARAM4) % Osc F
    ...
    fg=figure;  stylelst= {'-', ':', '--', '.-'};
    hold on;
    for ii = 1 : length(PARAM4)
        plot( VL_baselineWT(ii,:), [stylelst{ii} 'k']);  hold on;  plot( VL_baselineKO(1,:), [stylelst{ii} 'r']);
        title(ii)
