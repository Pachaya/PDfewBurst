...
BurstSpkTrain = zeros(nE+nI,length(PhotoStop+1 + DelayT : PhotoStop+1 + DelayT+BurstRange));
BurstSpkETrain = zeros(nE,length(PhotoStop+1 + DelayT : PhotoStop+1 + DelayT+BurstRange));
BurstSpkITrain = zeros(nI,length(PhotoStop+1 + DelayT : PhotoStop+1+ DelayT+BurstRange));
...
for ii = 1:nE
    ...
    tmpT = PhotoStop+1+ DelayT : PhotoStop+1+ DelayT+BurstRange;
    ...
    tmpT = PhotoStop+1+ DelayT: Tstop;
    fr_AfterLightOffE(ii) = sum(E.spktrain(ii,tmpT))/ length(tmpT)*1000;  
...
All.BurstSpkTrain = BurstSpkTrain;
...
E.BurstSpkTrain = BurstSpkETrain;
...
I.BurstSpkTrain = BurstSpkITrain;
