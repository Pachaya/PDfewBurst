...
dirLoc = 'Model1_MinParam150324_fix/' ;
...
sct_fg = figure; set(sct_fg,'position',[ 677   313   582   435]);  set(gcf,'PaperPositionMode','auto')
scatter(WTWT_MS(:,1),WTWT_MS(:,2),'k');
...
tmp_MS = [WTWT_MS; KOKO_MS];
 p = polyfit(tmp_MS(:,1),tmp_MS(:,2),2);
   ...
   psearch = [ -0.0223 1.4245 7.3833];
yyy = polyval(psearch,xx); %parameter search result
plot(xx, yyy,'k:','linewidth',2)
disp(['Parameter Search Curve :  y = ' num2str(psearch(1)) 'x^2 +' num2str(psearch(2)) 'x +' num2str(psearch(3)) ]) 
   binsize = 25;
...
%        a =       11.43  (9.78, 13.09)
%        b =     0.06084  (0.05784, 0.06384)

yfit = 0.1881.*xx.*xx + 1.5709.*xx ;
...
for TRIAL_NO = 1:NUM_TRIAL
    ...
    WT_in = dlmread(['PoisInputFR150324_fix_WT_' num2str(TRIAL_NO) '.txt']); WT_in = WT_in(2:end);
    tmp = ACT_Record{TRIAL_NO}.WT.All.fr_data;
    scatter(tmp,WT_in,'MarkerEdgeColor',[0 0 ccc(TRIAL_NO)]);
    LEG{2*TRIAL_NO} = ['WT, trial#' num2str(TRIAL_NO)];
    k = waitforbuttonpress;
    
    KO_in = dlmread(['PoisInputFR150324_fix_KO_' num2str(TRIAL_NO) '.txt']); KO_in = KO_in(2:end);
    tmp = ACT_Record{TRIAL_NO}.KO.All.fr_data;
